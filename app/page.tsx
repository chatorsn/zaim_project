      ...
      <div className="relative z-10">
        <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-[#E8E0D5]">
          <div className="max-w-7xl mx-auto px-6 py-5 flex justify-between items-center">
            ...
            {userId ? (
              <Link href="/account" className="text-[#2E5A4C] border border-[#2E5A4C] px-5 py-2 rounded-full hover:bg-[#2E5A4C] hover:text-white transition">Личный кабинет</Link>
            ) : (
          ...
          <div className="max-w-7xl mx-auto">
            <div className="max-w-3xl">
              ...
              <div className="flex flex-wrap gap-4">
                {userId ? (
                  <Link href="/account" className="bg-[#2E5A4C] text-white px-8 py-4 rounded-full font-medium">Личный кабинет</Link>
                ) : (
                  <Link href="/login" className="bg-[#2E5A4C] text-white px-8 py-4 rounded-full font-medium">Получить займ</Link>
                )}
                <Link href="/how-it-works" className="bg-white/20 backdrop-blur border border-white/30 text-white px-8 py-4 rounded-full font-medium">Как работает</Link>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 px-6">
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-10 bg-white/20 backdrop-blur border border-white/20 rounded-3xl p-8 lg:p-12">
              <div>
                <p className="text-white/70 mb-2">Кредитный калькулятор</p>
                <h2 className="text-3xl lg:text-4xl font-medium text-white mb-4">Рассчитайте условия</h2>
                <p className="text-white/70 text-sm">Ставка 0.8% в день</p>
              </div>
              <div className="space-y-6">
                ...
                <button onClick={calculate} disabled={loading} className="w-full bg-[#2E5A4C] text-white py-3 rounded-full font-medium disabled:opacity-50">
                  {loading ? 'Расчёт...' : 'Рассчитать'}
                </button>
                {result && (
                  <div className="bg-white/30 backdrop-blur rounded-xl p-5">
                    <div className="flex justify-between mb-3">
                      <span className="text-white/70">Ежедневный платёж</span>
        ...
        <footer className="border-t border-white/20 py-12 px-6 mt-8">
          <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-8">
            <div><h3 className="text-white font-semibold mb-4">LumenBridge Finance</h3></div>
            <div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-white/60">
                ...
                <li><Link href="/cookies">Cookie</Link></li>
              </ul>
            </div>
            <div>
              ...
              <p className="text-sm text-[#2E5A4C]">support@lumenbridge.example</p>
            </div>
          </div>
        </footer>
      </div>
